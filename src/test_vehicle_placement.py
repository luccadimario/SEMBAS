...
def test_open_loop_adjustment():
    ...
    test_cases = [
        ...
        (
            ...
            np.pi / 2,
            np.pi / 2,
        ),  # Start of lane, middle of lane, angle_offset = pi/2 -> pi/2
        ...
        (
            ...
            -np.pi / 2,
            -np.pi / 2,
        ),  # End of lane, middle of lane, angle_offset = -pi/2 -> -pi/2
        ...
        (
            ...
            -3 * np.pi / 2,
            -np.pi / 2,
        ),  # Start of lane, left edge, angle_offset = -3pi/2 -> -pi/2
        (
            1,
            1,
            3 * np.pi / 2,
            np.pi / 2,
        ),  # End of lane, right edge, angle_offset = 3pi/2 -> pi/2
        (
            0.5,
            0.5,
            np.pi / 4,
            np.pi / 4,
        ),  # Middle of lane, middle of lane, angle_offset = pi/4 -> pi/4
        (
            0.5,
            0.5,
            -np.pi / 4,
        ...
        (
            ...
            -3 * np.pi / 4,
            -np.pi / 2,
        ),  # Start of lane, middle of lane, angle_offset = -3pi/4 -> -pi/2
        (
            1,
            0.5,
            3 * np.pi / 4,
            np.pi / 2,
        ),  # End of lane, middle of lane, angle_offset = 3pi/4 -> pi/2
        (
            0,
            0,
            -np.pi / 2,
            -np.pi / 2,
        ),  # Start of lane, left edge, angle_offset = -pi/2 -> -pi/2
        (
            1,
            1,
            np.pi / 2,
            np.pi / 2,
        ),  # End of lane, right edge, angle_offset = pi/2 -> pi/2
    ]
